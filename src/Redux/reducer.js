...
function formatData(items) {
  let tempItems = Object.values(items).map((item) => {
    ...
    let room = { id, type: item.fields.type, price: item.fields.price }; // Only keep necessary fields
    return room;
  });
  return tempItems;
}
...
const reducer = (state = initialState, { type, payload }) => {
  switch (type) {
    case READ: {
      ...
      rooms = temp;
      let featuredRooms = rooms.filter((room) => room.featured === true); // Consider limiting featured rooms if necessary
